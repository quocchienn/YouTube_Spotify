...
const JS_NAME = 'Script Hub: Viết lại chuyển đổi'
...
//Cài đặt thời gian chờ và script-converter.js Tương tự
...
//$.log("Liên kết gốc：" + req);
const urlArg = url.split(/\/_end_\//)[1]

//Nhận các thông số
const queryObject = parseQueryString(urlArg)
//$.log("Tham số:" + $.toStr(queryObject));
...
let noNtf = queryObject.noNtf ? istrue(queryObject.noNtf) : false //Bật thông báo theo mặc định

let localsetNtf = $.lodash_get(arg, 'Notify') || $.getval('ScriptHub Thông báo') || ''
noNtf = localsetNtf == 'Bật thông báo' ? false : localsetNtf == 'Tắt thông báo' ? true : noNtf

let openMsgHtml = istrue(queryObject.openMsgHtml)

noNtf = openMsgHtml ? true : noNtf

let nName = queryObject.n != undefined ? getArgArr(queryObject.n) : null //Giới thiệu tên
let category = queryObject.category ?? null
let icon = queryObject.icon ?? null
let Pin0 = queryObject.y != undefined ? getArgArr(queryObject.y) : null //Giữ
let Pout0 = queryObject.x != undefined ? getArgArr(queryObject.x) : null //Loại trừ
let hnAdd = queryObject.hnadd != undefined ? queryObject.hnadd.split(/\s*,\s*/) : null //Đặt một thứ lên trên một thứ khác
let hnDel = queryObject.hndel != undefined ? queryObject.hndel.split(/\s*,\s*/) : null //Trừ
let hnRegDel = queryObject.hnregdel != undefined ? new RegExp(queryObject.hnregdel) : null //Xóa thường xuyên hostname
