const TITLE = `Script Hub: Chuyển đổi kịch bản `
...
!(async () => {
  if (!$.isRequest()) throw new Error('Không request')
  ...
  console.log('Tham số:' + JSON.stringify(queryObject))
  ...
  if (Object.keys(setHeaders).length > 0) {
    $.log(`Chỉ headers`, $.toStr(setHeaders))
  } else if (setContentType) {
    $.log('Chỉ Content-Type', setContentType)
  }

  let prefix = `
// Thời gian chuyển đổi: ${new Date().toLocaleString('zh')}
// Chuyển đổi tương thích
